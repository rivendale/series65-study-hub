...
The content is original and researched, but it has **not yet been reviewed by a licensed professional**. Where the sources ran out — study material disagreeing with primary sources, an inflation-indexed figure, a rule that varies by adopting state, or a judgment call reasonable advisors would make differently — the point is flagged rather than asserted flatly.

<!-- review-counts:start (generated by `npm run review:md` — do not edit by hand) -->
**29 items are currently flagged**, spanning 38 questions across 17 chapters.
<!-- review-counts:end -->

Each one records what the app tells a student, why it was flagged, and the specific thing to check. Both [`REVIEW.md`](./REVIEW.md) and the in-app `/review` page render from `src/data/reviewItems.ts`, so the two cannot drift apart.

Corrections are welcome — see [Reviewing content](./CONTRIBUTING.md#reviewing-content).
...
│   ├── reviewItems.ts    # content flagged for licensed review (see above)
...
│                         # Progress, Glossary, Formulas, CheatSheet,
│                         # Review, About
