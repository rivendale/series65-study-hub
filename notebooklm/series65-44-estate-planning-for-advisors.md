...
*This material is not tested on the Series 65 exam. It is here because estate planning is a principal knowledge area of the CFP® certification, and because advisors are usually the first professional to notice that a client's documents are stale, missing, or contradicted by their beneficiary forms.*
...
- Trust: Revocable living trust. Primary purpose: Probate avoidance, privacy, seamless incapacity management.
- Trust: Irrevocable life insurance trust (ILIT). Primary purpose: Keeps a death benefit out of the insured's gross estate and provides estate liquidity.
- Trust: Credit shelter / bypass trust. Primary purpose: Uses the first spouse's exclusion while providing for the survivor; assets are excluded from the survivor's estate.
- Trust: Marital / QTIP trust. Primary purpose: Qualifies for the marital deduction while the grantor controls the ultimate remainder — common in blended families.
- Trust: Special needs trust. Primary purpose: Supplements a beneficiary with a disability without disqualifying them from means-tested benefits.
- Trust: Charitable remainder trust (CRT). Primary purpose: Income stream to the donor or family for a term or life, remainder to charity; partial deduction and gain deferral on funding.
- Trust: Charitable lead trust (CLT). Primary purpose: Income to charity for a term, remainder to family.
- Trust: Dynasty trust. Primary purpose: Multi-generational wealth transfer leveraging the GST exemption, where state law permits long or perpetual duration.
- Trust: Grantor trust. Primary purpose: Grantor pays the income tax on trust income, effectively an additional tax-free transfer to beneficiaries.
...
**A couple in their thirties with two young children and modest assets have no estate documents at all. If both died in an accident, the most serious consequence would be that**

For a young family the guardian nomination is usually the single most important line in the will, and it is the one thing no beneficiary form or joint title can supply. Intestacy statutes distribute property in a fixed order that may not match intent, but property can be sorted out later; guardianship cannot. The cost of doing nothing here is not a tax cost.

**A client divorced eight years ago and remarried. His will leaves everything to his current wife, but the beneficiary form on his traditional IRA still names his ex-wife. At his death the IRA will**

For a typical household the will governs a minority of the wealth; retirement accounts, life insurance, annuities, and transfer-on-death registrations all pass by contract and override it. Auditing primary and contingent designations after every divorce, death, and birth is one of the cheapest and highest-value habits an advisor has. Some state statutes revoke an ex-spouse designation, but relying on that is a gamble and federal law preempts it for many plans.

**A client paid an attorney to prepare a revocable living trust three years ago. Her home is still titled in her individual name and her brokerage account is still registered individually. At her death, those assets will**

An unfunded trust is an expensive folder. Probate avoidance requires retitling — deed, account registration, and beneficiary coordination — which is exactly the follow-through advisors are best placed to catch. A pour-over will can move stray assets into the trust, but only by first passing them through probate. The step-up in basis is unaffected either way.

**A widowed client wants to add her adult son as a joint owner with right of survivorship on her $800,000 taxable brokerage account so it will avoid probate. The advisor should point out that**

This is among the most common do-it-yourself estate mistakes, and it is entirely avoidable because a TOD registration is free and takes ten minutes. Joint titling with a child converts an inheritance into a lifetime gift, drags the account into the child financial life, and gives up basis step-up on the portion treated as the child own. Joint tenancy does avoid probate, which is why the mistake is so tempting.

**A client with a $2.5 million estate — far below the federal exclusion — wants to give her daughter $200,000 of stock that has a $20,000 cost basis. The advisor should explain that**

For most families today the binding constraint is income tax basis, not estate tax. Gifting appreciated property hands the donee the donor basis and the entire embedded gain, while dying with it erases the gain through the step-up. Gifts above the annual exclusion require a gift tax return but use lifetime exclusion rather than generating current tax for an estate this size, and gifts are never income to the recipient.

**The first spouse in a married couple dies with an estate well below the federal exclusion, and no estate tax is due. To preserve the deceased spousal unused exclusion for the survivor, the estate must**

Portability is an election, not a default, and it is missed constantly precisely because families see no tax and assume no filing. The return is the only way to capture the deceased spouse unused exclusion, which can matter enormously if the survivor later remarries, sells a business, or if exclusion amounts fall. Note that the GST exemption is not portable, which is one reason credit shelter trusts still have a role.
