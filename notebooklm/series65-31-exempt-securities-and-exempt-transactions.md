...
Municipal securities are exempt securities because the exemption attaches to the instrument itself. Unsolicited orders and small private placements are exempt TRANSACTIONS, which attach to the circumstances of a particular sale, and bank holding company stock is not a bank security.
...
United States, state, municipal and Canadian government securities, bank and savings institution securities, and securities of nonprofit religious, charitable and educational organizations are all exempt securities. A bank holding company is not a bank, so its stock enjoys no exemption.

**For commercial paper to qualify as an exempt security under the Uniform Securities Act, the paper must:**

The commercial paper exemption under the Uniform Securities Act requires all three conditions: a maturity of nine months or less, a minimum denomination of $50,000, and a rating in one of the three highest categories. Paper that fails any single condition is not an exempt security under state law. Do not import the federal test here — the Securities Act exemption turns on maturity and use of proceeds for current transactions and sets no denomination minimum, so the two tests are answered separately.

**A customer telephones her agent and directs him to buy 500 shares of an unregistered, non-exempt security that the agent has never discussed with her. The agent enters the order. This is:**

The unsolicited non-issuer transaction is the most commonly tested exempt transaction, and it turns on who initiated the order rather than on the security or the wealth of the customer. Firms ordinarily document the exemption with a customer-signed acknowledgment that the order was unsolicited.

**Under the private placement exemption in the Uniform Securities Act, an issuer may make offers to:**

The model act exempts a transaction pursuant to an offer directed to not more than ten persons other than institutional investors during any twelve consecutive months, so the count runs on offerees rather than on completed sales and offers to institutions are unlimited. The seller must also reasonably believe the non-institutional buyers are buying for investment rather than resale, and may pay no commission for soliciting them. The 35-purchaser figure belongs to federal Regulation D, not to the Uniform Securities Act.

**Which of the following qualifies for the exempt transaction covering sales by a fiduciary?**

The exemption reaches fiduciaries appointed by and accountable to a court: executors, administrators, sheriffs, marshals, receivers, trustees in bankruptcy, guardians and conservators. A trustee of an inter vivos trust is appointed by the grantor rather than by a court, so those sales fall outside the exemption.
